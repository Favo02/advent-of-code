...
func parseInput() (nums []Number) {
	...
	for scanner.Scan() {
		...
		nums = append(nums, Number{n * DECRYPTION_KEY, i})
		i++
	}
	return nums
}
...
func moveNumber(nums []Number, index, value int) []Number {
	if value == 0 {
		...
		return doForwardSwaps(nums, index, value%(len(nums)-1))
	} else {
		return doBackwardSwaps(nums, index, value%(len(nums)-1))
	}
}
